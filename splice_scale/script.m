%%CSL-603 Project.
A = svm2mat('splice_scale.txt'); % Extracting the training data.
X = A(:, 2:end); % Getting the Training data.
Y = A(:, 1); % Getting the class label.
B = svm2mat('splice.t'); % Extracting the testing data.
...
for i = 1: size(predY, 1)
   if(predY(i, 1) == Y(i, 1))
      acc_train(sg2) = acc_train(sg2) + 1; 
   end
end
acc_train(sg2) = acc_train(sg2) / size(Y,1);
acc_train(sg2) = acc_train(sg2) * 100;
...
for i = 1: size(predictionY, 1)
   if(predictionY(i, 1) == YTest(i, 1))
      acc_test(sg2) = acc_test(sg2) + 1; 
   end
end
acc_test(sg2) = acc_test(sg2) / size(YTest, 1);
acc_test(sg2) = (acc_test(sg2) * 100);
disp('The accuracy on the test data.');
disp(acc_test(sg2));
end
figure,
